        ...
        <div className="w-1/2 flex flex-col border-r border-zinc-800 min-h-0">
          ...
          <div className="flex flex-col h-[35%] min-h-0 border-b border-zinc-800">
            ...
            <textarea
              ...
              spellCheck={false}
              className="flex-1 w-full resize-none bg-transparent p-3 font-mono text-[13px] leading-relaxed text-emerald-400 outline-none min-h-0"
            />
          </div>
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between px-3 py-1.5 bg-zinc-900/50 border-b border-zinc-800 flex-shrink-0">
              ...
              <button
                onClick={runScript}
                disabled={isRunning}
                className="px-3 py-1 text-[11px] font-semibold uppercase tracking-wider bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 hover:bg-emerald-500/20 disabled:opacity-40 disabled:cursor-not-allowed transition-colors rounded"
              >
          ...
          <div className="flex-shrink-0 border-b border-zinc-800 bg-zinc-900/30">
            <div className="flex items-center gap-2 px-3 py-2">
              ...
              <input
                ...
                disabled={isRunning}
                className="flex-1 bg-zinc-950 border border-zinc-800 rounded px-2 py-1 text-[13px] font-mono text-zinc-200 outline-none focus:border-emerald-500/40 placeholder:text-zinc-700 disabled:opacity-50"
              />
              <button
            ...
            <div className="flex items-center gap-1.5 px-3 pb-2 flex-wrap">
              ...
              {QUICK_ACTIONS.map((qa) => (
                <button
                  ...
                  disabled={isRunning}
                  className="px-2 py-0.5 text-[10px] font-mono bg-zinc-800/60 text-zinc-400 border border-zinc-700/50 rounded hover:text-emerald-400 hover:border-emerald-500/30 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  + {qa.label}
                </button>
              ))}
