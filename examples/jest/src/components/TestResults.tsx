import type { TestResult } from '../hooks/useJestRunner';
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-zinc-600 border-t-orange-400" />
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center text-zinc-600">
          ...
          <p className="text-xs mt-1">Click "Run Tests" to execute</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between px-3 py-1.5 bg-zinc-900/50 border-b border-zinc-800">
        <span className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500">Test Results</span>
                ...
                {tests.map((t, i) => (
                  <div key={i} className="flex items-start gap-2 px-3 py-1.5 text-[12px]">
                    ...
                    <div className="min-w-0">
                      <span className={t.status === 'pass' ? 'text-zinc-300' : 'text-red-300'}>{t.name}</span>
                      {t.error && <div className="mt-0.5 text-[11px] text-red-400/70 font-mono break-all">{t.error}</div>}
                    </div>
                  </div>
                ))}
              </div>
